...
pyserial-or-pywin32 (see optiscan.py), WinLase via pywin32 (win32com).
    ...
    python tools/dice_wafer.py <set> --arm --focus --loops 175 --home-after
"""
...
def print_plan(set_dir: Path, plan, loops: int, focus: bool, armed: bool) -> None:
    print("\nDicing plan for %s   [%s, %d loops%s]" % (
        set_dir.name, "ARMED - LASER LIVE" if armed else "SIMULATE - no laser",
        loops, ", focus on" if focus else ""))
...
def main() -> int:
    ...
    p.add_argument("--positions", type=Path, default=Path("tools/optiscan_positions.json"))
    p.add_argument("--port", default="COM5")
    p.add_argument("--loops", type=int, default=DEFAULT_LOOPS, help="mark passes per station")
    ...
    print_plan(args.set_dir, plan, args.loops, args.focus, args.arm)
    if args.list:
        return 0
    ...
    try:
        ...
        for label, pos, wlj in plan:
            ...
            if abort():
                ...
                break
            if args.arm:
                print("[%s] marking %s x%d ..." % (label, wlj.name, args.loops))
                if not marker.mark_job(wlj, args.loops, abort):
                    print("aborted during %s." % label)
                    break
            else:
                print("[%s] SIMULATE mark %s x%d (no laser)" % (label, wlj.name, args.loops))
                time.sleep(1.0)
            print("[%s] done." % label)
        else:
