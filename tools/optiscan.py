...
class OptiScan:
    def __init__(self, port: str = DEFAULT_PORT, baud: int = BAUD):
        ...
        # Standard mode + human-readable errors. Confirm the link is really talking.
        self.command("COMP,0")
        self.command("ERROR,1")
    ...
    def command(self, cmd: str, multiline: bool = False) -> str:
        """Send one command, return the reply. Raises on an 'E'rror reply."""
        ...
        if reply[:1] == "E" and (len(reply) == 1 or reply[1:].lstrip("-").isdigit()
                                 or reply.upper().startswith("ERROR")):
            raise RuntimeError("OptiScan error for %r: %r" % (cmd, reply))
        return reply

    # -- reads --------------------------------------------------------------------
    def stage_position(self):
        x, y = (int(round(float(v))) for v in self.command("PS").split(","))
        return x, y
    ...
    def is_moving(self) -> bool:
        try:
            return int(self.command("$").split(",")[0]) != 0
        except ValueError:
            return False
    ...
    def goto(self, x: int, y: int, wait: bool = True) -> None:
        self.command("G,%d,%d" % (int(x), int(y)))
        if wait:
            self.wait_idle()

    def goto_z(self, z: int, wait: bool = True) -> None:
        self.command("GZ,%d" % int(z))
        if wait:
            self.wait_idle()

    def move_rel(self, dx: int, dy: int, wait: bool = True) -> None:
        self.command("GR,%d,%d" % (int(dx), int(dy)))
        if wait:
            self.wait_idle()

    def move_rel_z(self, dz: int, wait: bool = True) -> None:
        self.command("GR,0,0,%d" % int(dz))
        if wait:
            self.wait_idle()
...
def cmd_home(args) -> int:
    dev = _connect(args)
    try:
        print("M moves stage AND focus to 0,0,0.")
        if not args.yes and input("proceed? [y/N] ").strip().lower() != "y":
            print("cancelled")
            return 1
        dev.command("M")
...
def main() -> int:
    ...
    j.add_argument("--out", default="tools/optiscan_positions.json",
