...
# 2026-08-20 NEW-JIG calibration iteration 1 (RETIRED): the v6 seam test on the +X-major jig
# gave X=+4850 / Y=-3207 um (kept in git history if the +X jig ever returns). RESET TO 0 for
# the +Y-flat nest: that +X-jig offset does not apply, and the full-range decoupled dicing
# calibrates via the TAUGHT STATIONS, not a baked DXF offset -- baking it into a full-field job
# pushes far-edge content past the galvo (see the galvo-reach guard). Re-tune per jig by
# re-teaching P1-P4; override at build with --global-x/--global-y if ever needed.
