...
- All cutting geometry is on literal DXF layer `0` with millimeter drawing units.
- Four 50 um registration anchors are on `REGISTRATION_DO_NOT_EXPOSE`. They
  force every file to have the identical `(-30,-30) to (+30,+30) mm` bounding
  box and prevent the DXF22 alignment marker from changing automatic placement.

## Critical laser-import setting

Set `REGISTRATION_DO_NOT_EXPOSE` to no marking / zero power before running any
file. Never expose that layer. Layer `0` is the only cutting layer.

The registration layer makes per-file automatic centering geometrically stable,
but importing at the original drawing origin `(0,0)` remains preferred. Do not
use a fit-to-field operation that scales the drawing.
